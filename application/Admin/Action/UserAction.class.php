...
class UserAction extends AdminbaseAction{
	...
	protected $users_obj_es,$users_obj_fr,$users_obj_pt,$users_obj_en,$users_obj_ar,$users_obj_de,$users_obj_kr,$users_obj_tw,$users_obj_cmf;
	...
	function add_post(){
		if(IS_POST){
			...
			if($en && $es && $fr && $pt && $ru && $ar && $de && $kr && $tw && $cmf){
				if($this->users_obj->add()!==false && $this->users_obj_es->add()!==false && $this->users_obj_fr->add()!==false && $this->users_obj_pt->add()!==false && $this->users_obj_en->add()!==false && $this->users_obj_ar->add()!==false && $this->users_obj_de->add()!==false && $this->users_obj_kr->add()!==false && $this->users_obj_tw->add()!==false && $this->users_obj_cmf->add()!==false){
	...
	function edit_post(){
		if (IS_POST) {
			...
			if ($en && $ar && $es && $fr && $pt && $ru && $de && $kr && $tw && $cmf) {
				...
				if ($result!==false && $es!==false && $fr!==false && $pt!==false && $en!==false && $ar!==false && $de!==false && $kr!==false && $tw!==false && $cmf!==false) {
					$this->success("保存成功！");
				} else {
					$this->error("保存失败！");
				}
	...
	function delete(){
		...
		if ($this->users_obj->where("id=$id")->delete()!==false && $this->users_obj_es->where("id=$id")->delete()!==false && $this->users_obj_fr->where("id=$id")->delete()!==false && $this->users_obj_pt->where("id=$id")->delete()!==false && $this->users_obj_en->where("id=$id")->delete()!==false && $this->users_obj_ar->where("id=$id")->delete()!==false && $this->users_obj_de->where("id=$id")->delete()!==false && $this->users_obj_kr->where("id=$id")->delete()!==false && $this->users_obj_tw->where("id=$id")->delete()!==false && $this->users_obj_cmf->where("id=$id")->delete()!==false) {
			$this->success("删除成功！");
		} else {
			$this->error("删除失败！");
		}
